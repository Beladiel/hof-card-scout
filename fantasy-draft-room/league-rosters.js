(() => {
  ...
  function normalizeHomeTeam(draft) {
    ...
    if (prior && prior !== slot && meta.teamNames[prior-1] === "Lobstahs") meta.teamNames[prior-1] = `Team ${prior}`;
    meta.teamNames[slot-1] = "Lobstahs";
  ...
  function assignPlayer(rawName, teamNo, pos) {
    ...
    if (!known) {
      meta.customPlayers[name] = {...(meta.customPlayers[name] || {}), pos:pos || "WR", owner:teamNo};
    }
  ...
  function render() {
    ...
    const toolbar = `
      ...
      <div class="lr-hint">${homeSlot ? `🦞 Lobstahs are Team ${homeSlot}. Drafted players will appear here automatically.` : `Set the Lobstahs draft slot at the top before your mock; Scout will then identify your roster automatically.`}</div>`;
    ...
    const cards = Array.from({length:TEAMS},(_,i)=>{
      ...
      return `<article class="lr-team ${home ? "is-home" : ""}">
        <div class="lr-team-head">
          <div class="lr-team-num">${home ? "🦞 " : ""}TEAM ${n}</div>
    ...
    const assignBtn = document.getElementById("lrAssignBtn");
    ...
    root.querySelectorAll("[data-team-name]").forEach(el => el.addEventListener("change", ()=>{
      ...
      if (n === homeSlot) meta.teamNames[n-1] = "Lobstahs";
      saveMeta(); render();
    }));
  ...
  slot?.addEventListener("change", ()=>setTimeout(render,0));
