(() => {
  ...
  function updateMissedButton() {
    ...
    btn.setAttribute("aria-disabled", selectedName ? "false" : "true");
  }
  ...
  function insertSelectedMissedPick() {
    ...
    if (!selectedName) {
      alert("Tap a player on the Best Available board first.");
      return;
    }
  ...
  function injectStyle() {
    if (document.getElementById("draftControlsV44Style")) return;
    const style = document.createElement("style");
    style.id = "draftControlsV44Style";
    style.textContent = `
      #playerBoard .player-row{cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent;transition:border-color .12s ease,background .12s ease,box-shadow .12s ease}
      ...
      .board-tools-v44{display:grid;gap:7px}.board-quick-actions{display:flex;gap:6px;flex-wrap:wrap}.board-quick-actions button{touch-action:manipulation}
      @media(max-width:620px){.board-quick-actions{display:grid;grid-template-columns:1fr 1fr}.board-quick-actions #missedPickBtn{grid-column:1/-1}}
    `;
  ...
  if (window.PointerEvent) {
    document.addEventListener("pointerup", handleRowPointer, {passive:true});
  } else {
  ...
  document.addEventListener("click", event => {
    ...
    const clear = event.target.closest?.("#clearSearchBtn");
    if (clear) {
      event.preventDefault();
      clearSearch();
      return;
    }
    const undo = event.target.closest?.("#boardUndoBtn");
    if (undo) {
      event.preventDefault();
  ...
  injectStyle();
  document.querySelector(".board-tools")?.classList.add("board-tools-v44");
  setupRosterCollapse();
  applySelection();
  enforceMineCap();
  enforceBackupTeAlert();
