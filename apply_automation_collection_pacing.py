...
base_old="const baseState={settings:{monthlySerpCap:30,targetMonitoringEnabled:true,targetCadenceDays:7,collectionRefreshEnabled:true,collectionCardsPerMonth:10},usage:{period,serpSuccessful:0,cardApiRequests:0,apifyRuns:0,collectionCardsChecked:0},alerts:[],targetChecks:{},collectionChecks:{}};"
base_new="const baseState={settings:{monthlySerpCap:30,targetMonitoringEnabled:true,targetCadenceDays:7,collectionRefreshEnabled:true,collectionCardsPerMonth:10},usage:{period,serpSuccessful:0,cardApiRequests:0,apifyRuns:0,collectionCardsChecked:0},alerts:[],targetChecks:{},collectionChecks:{},collectionCooldownUntil:''};\nfunction freshState(){return {...baseState,settings:{...baseState.settings},usage:{...baseState.usage},targetChecks:{},collectionChecks:{},collectionCooldownUntil:''};}"
assert base_old in t
t=t.replace(base_old,base_new,1)
...
t=t.replace("runOneAutomationCollectionCheck({SERPAPI_KEY:'private-key'},baseState,catalog,new Date())", "runOneAutomationCollectionCheck({SERPAPI_KEY:'private-key'},freshState(),catalog,new Date())")
