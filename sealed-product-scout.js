  ...
  async function runShelfShowdown(){
    ...
    for(let i=0;i<items.length;i++){
      ...
      try{
        ...
        const marketData=await marketRes.json().catch(()=>({}));marketSearches+=Number(marketData.marketplaceSearchesUsed||0);if(!marketRes.ok||!marketData.ok)throw new Error(marketData.message||"Market-price research failed.");
        ...
        if(Number.isFinite(market.median)&&market.median>0){
          try{
            ...
            const ripData=await ripRes.json().catch(()=>({}));researchSearches+=Number(ripData.researchSearchesUsed||0);
    ...
    const compact=researched.map(row=>({status:row.metrics?.rankable?"ranked":"incomplete",item:row.item,market:row.market,analysis:{priceScore:row.analysis?.priceScore,chaseDepthScore:row.analysis?.chaseDepthScore,chaseDepthEvidenceAvailable:!!row.analysis?.chaseDepthEvidenceAvailable,chaseDepthLabel:row.analysis?.chaseDepthLabel||"",chaseDepthSummary:row.analysis?.chaseDepthSummary||"",chaseDepthCount20:Number(row.analysis?.chaseDepthCount20||0),chaseDepthCount50:Number(row.analysis?.chaseDepthCount50||0),chaseDepthCount100:Number(row.analysis?.chaseDepthCount100||0),chaseScore:row.analysis?.chaseScore,chaseEvidenceAvailable:!!row.analysis?.chaseEvidenceAvailable,formatAccessScore:row.analysis?.formatAccessScore,formatAccessEvidenceAvailable:!!row.analysis?.formatAccessEvidenceAvailable,formatAccessSummary:row.analysis?.formatAccessSummary||"",pullScore:row.analysis?.pullScore,pullEvidenceAvailable:!!row.analysis?.pullEvidenceAvailable,sentimentScore:row.analysis?.sentimentScore,sentimentEvidenceAvailable:!!row.analysis?.sentimentEvidenceAvailable,qualitySummary:row.analysis?.qualitySummary||"",researchProfile:row.analysis?.researchProfile||"",researchMode:row.analysis?.researchMode||"",lanes:row.analysis?.lanes||{},failureStage:row.analysis?.failureStage||"",researchMix:row.analysis?.researchMix||{}},metrics:row.metrics,error:row.error}));
