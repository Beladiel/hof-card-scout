...
assert.match(worker,/const VERSION = "3\.27\.0";/);
...
assert.match(worker,/runnerEnabled: false/,'background runner must remain off during the guardrail-only gate');
assert.doesNotMatch(worker,/scheduled\s*\(/,'guardrail gate must not silently enable cron automation');
