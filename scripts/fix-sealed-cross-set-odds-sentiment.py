...
'''
replacement = '''function sealedRipOddsSupported(odds, evidenceText) {
  const raw = String(odds || "").trim();
  if (!raw) return false;
  const simplify = value => String(value || "").toLowerCase().replace(/\\s+/g, "").replace(/[–—]/g, "-");
  return simplify(evidenceText).includes(simplify(raw));
...
function sealedRipOddsRowSupported(row, evidenceRows, identity = {}) {
  ...
  return (Array.isArray(evidenceRows) ? evidenceRows : []).some(source => {
    ...
    const oddsAt = sourceText.indexOf(odds, Math.max(0, itemAt - 700));
...
'''
repl(anchor, replacement, 'odds evidence helpers')

repl('''function sealedRipNormalize(raw, evidenceRows, market, identity = {}) {
''', '''function sealedRipNormalize(raw, evidenceRows, market, identity = {}, productLabel = "") {
''', 'normalize signature')
...
\\
High-signal excerpts extracted from the best sources (read these first):\\
''', 'prompt community separation')

repl('''const analysis = sealedRipNormalize(cached.analysis, Array.isArray(cached.evidenceRows) ? cached.evidenceRows : [], market, identity);''', '''const analysis = sealedRipNormalize(cached.analysis, Array.isArray(cached.evidenceRows) ? cached.evidenceRows : [], market, identity, productLabel);''', 'cached normalize call')
repl('''const analysis = sealedRipNormalize(aiObject, evidenceRows, market, identity);''', '''const analysis = sealedRipNormalize(aiObject, evidenceRows, market, identity, productLabel);''', 'fresh normalize call')
...
txt = txt.replace('assert.match(worker,/const VERSION = "3\\.40\\.2"/);', 'assert.match(worker,/const VERSION = "3\\.40\\.3"/);', 1)
txt = txt.replace("assert.match(worker,/sealed:intel:v4:/,'sealed product intelligence must use a reusable product cache');", "assert.match(worker,/sealed:intel:v5:/,'sealed product intelligence must use a reusable product cache');", 1)
